...
# Configuration
TOTAL_DIMENSIONS = 12000  # Total dimension budget
NUM_EXPERIMENTS = 100
NUM_ENSEMBLE_MODELS = 12 # Max models needed for seeds
...
# Load Data
print("Loading UCIHAR dataset...")
# Using ../data to be consistent with isolet script structure, or fallback to ./data if preferred.
# Assuming shared data directory structure.
...
def run_experiment():
    ...
    for i, seeds in enumerate(seed_groups):
        ...
        # --- Baseline Run (1 model, 12000 dim) ---
        ...
        # --- Ensemble 3 Run (3 models, 4000 dim each) ---
        ...
        # --- Ensemble 6 Run (6 models, 2000 dim each) ---
        ...
        # --- Ensemble 12 Run (12 models, 1000 dim each) ---
        ens12_dim = TOTAL_DIMENSIONS // 12
        ens12_seeds = seeds[:12]
        print(f"[Ensemble 12] Running with 12 models, dim per model={ens12_dim}")
        
        models_12 = []
        for seed in ens12_seeds:
            torch.manual_seed(seed)
            sub_model = Classifier(NUM_CLASSES, ens12_dim, INPUT_FEATURES, device=device)
            sub_model.fit(train_ld)
            models_12.append(sub_model)
            
        preds, labels = ensemble_predict(models_12, test_ld)
        acc = accuracy_score(labels, preds)
        ens12_accuracies.append(acc)
        print(f"[Ensemble 12] Accuracy: {acc:.4f}")
    ...
    mean_ens12 = statistics.mean(ens12_accuracies)
    std_ens12 = statistics.stdev(ens12_accuracies)
    ...
    print(f"Ensemble 12 (Dim={TOTAL_DIMENSIONS//12}): Mean={mean_ens12:.4f}, Std={std_ens12:.4f}")
    
    # Save CSV
    csv_filename = f"comparison_ucihar_1_3_6_12_models_{TOTAL_DIMENSIONS}.csv"
    csv_path = os.path.join(RESULTS_DIR, csv_filename)
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Experiment', 'Baseline_Acc', 'Ensemble3_Acc', 'Ensemble6_Acc', 'Ensemble12_Acc', 'Seeds'])
        for i in range(NUM_EXPERIMENTS):
            writer.writerow([i+1, baseline_accuracies[i], ens3_accuracies[i], ens6_accuracies[i], ens12_accuracies[i], seed_groups[i]])
        writer.writerow([])
        writer.writerow(['Average', mean_base, mean_ens3, mean_ens6, mean_ens12])
        writer.writerow(['StdDev', std_base, std_ens3, std_ens6, std_ens12])
