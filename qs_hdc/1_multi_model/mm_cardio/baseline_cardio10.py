...
import torchvision
from torchhd.datasets import Cardiotocography10Clases
...
OUTPUT_FILE = os.path.join(RESULTS_DIR, f"baseline_cardio_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")

# 确保目录存在
os.makedirs(RESULTS_DIR, exist_ok=True)
os.makedirs(DATA_DIR, exist_ok=True)

# Load Cardiotocography10Clases dataset
# We use fold 0 for train/test split
train_ds = Cardiotocography10Clases(DATA_DIR, train=True, fold=0, download=True)
test_ds = Cardiotocography10Clases(DATA_DIR, train=False, fold=0, download=True)
...
class Classifier(nn.Module):
    ...
    def encode(self, x: Tensor) -> torchhd.BSCTensor:
        ...
        # Data is min-max normalized to [0, 1], so -0.5 centers it to [-0.5, 0.5]
        x = x - 0.5
