...
OUTPUT_IMAGE = os.path.join(RESULTS_DIR, f"euro_comparison_plot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
...
baseline_csv = get_latest_csv("baseline_euro_results_")
ensemble_csv = get_latest_csv("ensemble_euro_results_")
...
if not baseline_csv or not ensemble_csv:
    ...
    print("Please run baseline_EuroLanguages.py and ensemble_EuroLanguages.py first.")
    if not baseline_csv:
...
plt.title('EuroLanguages: Baseline vs Ensemble Model Accuracy', fontsize=14)
