...
OUTPUT_IMAGE = os.path.join(RESULTS_DIR, f"emg_comparison_plot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
...
baseline_csv = get_latest_csv("baseline_emg_results_")
ensemble_csv = get_latest_csv("ensemble_emg_results_")
...
if not baseline_csv or not ensemble_csv:
    ...
    print("Please run baseline_emg.py and ensemble_emg.py first.")
...
plt.title('EMG: Baseline vs Ensemble Model Accuracy', fontsize=14)
