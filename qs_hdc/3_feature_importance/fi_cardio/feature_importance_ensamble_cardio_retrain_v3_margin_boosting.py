...
def run_experiment(
    ...
    dataset = Cardiotocography3Clases(data_dir, download=True)
    ...
    for r in range(num_runs):
        ...
        # Cardio specific: Split dataset into Train/Test (20% test)
        # Note: We use seed_base to ensure reproducibility of split
        train_val_subset, test_subset = split_train_val(
            dataset, val_ratio=0.2, seed=seed_base
        )
        
        # If val_ratio passed in args > 0 (e.g. 0.1), we split train_val_subset further
        # But for consistency with typical Cardio usage, we might just use train_val_subset as training
        # unless 'val_acc' weighting is requested.
        
        if weighting == "val_acc" or val_ratio > 0.0:
            # If explicit val_ratio provided or val_acc needed
            # We treat 'val_ratio' argument as ratio of TRAIN set to be used for validation
            # Current args.val_ratio default is 0.1
            train_subset, val_subset = split_train_val(train_val_subset, val_ratio=val_ratio, seed=seed_base)
        else:
        ...
        if val_subset:
            ...
            val_ld = None
        test_ld = torch.utils.data.DataLoader(test_subset, batch_size=1, shuffle=False)
        ...
        if weighting == "boosting":
            ...
            if weighting == "val_acc":
                if val_ld is None:
                    raise ValueError("Validation set required for 'val_acc' weighting.")
...
def main():
    ...
    # Note: args.D_total maps to --D in old script, args.per_expert_ratio maps to --rr roughly
    # The old script had: --D, --E, --runs, --rr, --epochs, --lr, --margin
    # New script matches HAR arguments: --D_total, --E, --num_runs, --val_ratio, --weighting, --common_ratio, --per_expert_ratio
    
    # User might call with old arguments if they have scripts, but since this is "retrain v3", likely following new standard.
    # However, to be safe, if I were preserving CLI, I would alias.
    # But user asked to "refer to logic" of HAR, which implies HAR's CLI structure too.
    ...
    importance_csv_path = os.path.normpath(
        os.path.join(parent_dir, "rf_feature_importance_results/cardio_feature_importance.csv")
    )
    ...
    filename = f"cardio_fi_D{args.D_total}_E{args.E}_{args.weighting}_rr{int(args.per_expert_ratio * 100)}"
    if args.margin > 0:
