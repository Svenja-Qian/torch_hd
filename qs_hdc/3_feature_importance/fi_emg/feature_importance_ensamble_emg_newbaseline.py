...
def run_experiment(
    ...
    base_acc_std = float(np.std(baseline_accs))
    ens_acc_std = float(np.std(ensemble_accs))
    ...
    print(f"Baseline mean accuracy: {base_acc_mean:.4f}, std: {base_acc_std:.4f}")
    print(f"Ensemble mean accuracy: {ens_acc_mean:.4f}, std: {ens_acc_std:.4f}")
    for i, acc in enumerate(expert_accs_mean):
