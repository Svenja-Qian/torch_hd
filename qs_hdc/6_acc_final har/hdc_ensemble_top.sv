...
module hdc_ensemble_top #(
    ...
    parameter int unsigned NUM_CLASSES      = 6,
    parameter int unsigned CHUNK_WIDTH      = 32,
    parameter int unsigned FEAT_WIDTH       = 16,
    // UCI HAR feature-importance partition (common_ratio=0.10, per_expert_ratio=0.30):
    // 561 total input features -> 56 shared + 112 private = 168 rows per expert.
    parameter int unsigned MAX_FEATURES     = 168,
    ...
    parameter int unsigned FEAT_IDX_W    = $clog2(MAX_FEATURES),                 // 8 at MAX_FEATURES=168
    parameter int unsigned FEAT_CNT_W    = $clog2(MAX_FEATURES + 1),             // 8 at MAX_FEATURES=168
    parameter int unsigned CLASS_IDX_W   = $clog2(NUM_CLASSES),                  // 3 at NUM_CLASSES=6
    ...
    generate
        for (genvar ge = 0; ge < NUM_EXPERTS; ge++) begin : gen_proj_bram
            // BRAM depth = MAX_FEATURES * NUM_CHUNKS (e.g., 168*64 = 10752)
