...
FI_HAR_DIR = QS_HDC_DIR / "3_feature_importance" / "fi_har"
if str(FI_HAR_DIR) not in sys.path:
    sys.path.insert(0, str(FI_HAR_DIR))

from feature_importance_ensamble_har_retrain_v3_margin_boosting import (  # noqa: E402
    INPUT_FEATURES,
    NUM_CLASSES,
    UCIHAR,
...
def quantize_to_int16_flat(sample: torch.Tensor, scale: float) -> torch.Tensor:
    sample_flat = sample.view(-1).float()
    sample_q = torch.round(sample_flat * scale)
    sample_q = torch.clamp(sample_q, -32768, 32767)
    return sample_q.to(torch.int16)
...
def compute_expected_class_for_sample(
    experts,
    sample_q: torch.Tensor,
    q_weights: list[int],
) -> int:
    weighted_dist = None
    sample_q_i32 = sample_q.to(torch.int32).cpu()
    with torch.no_grad():
        for expert_id, expert in enumerate(experts):
            feat_indices = expert.feature_indices.detach().cpu().long()
            sample_subset = sample_q_i32[feat_indices]
            weight_pm1 = torch.where(
                expert.projection.weight.detach().cpu() > 0,
                torch.tensor(1, dtype=torch.int32),
                torch.tensor(-1, dtype=torch.int32),
            )
            acc = torch.sum(weight_pm1 * sample_subset.unsqueeze(0), dim=1)
            hv = (acc > 0).to(torch.bool)
...
def export_testcases(
    ...
    raw_path = out_dir / "testcases_raw_features.mem"
    ...
    with raw_path.open("w", encoding="ascii") as raw_f, stream_path.open("w", encoding="ascii") as stream_f:
        for case_id in range(num_cases):
            ...
            sample_q = quantize_to_int16_flat(sample, quant_scale)
            expected_class = compute_expected_class_for_sample(experts, sample_q, q_weights)

            raw_f.write(f"{expected_class & 0xffff:04x}\n")
            for value in sample_q.tolist():
            ...
            for row_idx, raw_idx in enumerate(common_indices.tolist()):
                value = int(sample_q[raw_idx].item())
        ...
        json.dump(
            {
                "num_cases": num_cases,
                "offset": offset,
                "raw_features_file": raw_path.name,
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=str, default=str(CURRENT_DIR / "generated" / "har_hw_init"))
    ...
    parser.add_argument("--per-expert-ratio", type=float, default=0.30)
    ...
    importance_csv_path = QS_HDC_DIR / "3_feature_importance" / "rf_feature_importance_results" / "har_feature_importance.csv"
    data_dir = FI_HAR_DIR / "data"
    ...
    train_ld = torch.utils.data.DataLoader(train_subset, batch_size=args.batch_size, shuffle=True)
    train_eval_ld = torch.utils.data.DataLoader(train_subset, batch_size=args.batch_size, shuffle=False)
    val_ld = torch.utils.data.DataLoader(val_subset, batch_size=args.batch_size, shuffle=False)
    ...
    manifest = {
        "dataset": "ucihar",
        "preprocess": "raw_feature_vector_no_centering",
    ...
    if testcase_info is not None:
        manifest["files"]["testcase_raw_features"] = testcase_info["raw_features_file"]
    ...
    if testcase_info is not None:
        print(out_dir / testcase_info["raw_features_file"])
